...
def gradient_checkpointing_enable(self):
    ...
    if isinstance(self, Generator):
        ...
        # Store original forward methods for upsample blocks and replace with checkpointed versions
        for i, block in enumerate(self.upsample_blocks):
            original_forward = block.forward
            
            def make_checkpoint_fn(orig_fn):
                def checkpoint_fn(x):
                    return checkpoint.checkpoint(orig_fn, x)
                return checkpoint_fn
            
            block._orig_forward = original_forward
            block.forward = make_checkpoint_fn(original_forward)
        ...
        # Store original forward methods for downsample blocks and replace with checkpointed versions
